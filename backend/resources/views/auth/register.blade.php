...
<head>
  ...
  <title>Register – Andaya's Native Products</title>
...
<body class="bg-cream font-sans min-h-screen">
  ...
  <nav class="anim-1 fixed top-0 left-0 right-0 z-40 bg-bark/95 backdrop-blur-sm px-6 md:px-10 h-16 flex items-center justify-between shadow-sm">
    ...
    <div class="flex items-center gap-7">
      ...
      <a href="{{ route('login') }}" class="ml-2 px-5 py-2 border text-sm tracking-[0.15em] uppercase font-light transition-all duration-300 rounded-sm {{ request()->routeIs('login') ? 'bg-gold text-bark border-gold' : 'border-gold/50 text-gold hover:bg-gold hover:text-bark' }}">Sign In</a>
    </div>
  </nav>
  ...
  <main class="min-h-screen flex items-center justify-center px-5 pt-24 pb-10">
    ...
    <div class="relative hero-card rounded-2xl border border-bark/10 shadow-2xl shadow-bark/15 w-full max-w-lg px-12 py-14 overflow-hidden">
      ...
      <div class="anim-2 flex justify-center mb-6">
        <span class="divider-ornament text-gold text-xs tracking-[0.35em] uppercase font-sans font-medium">Sumali sa Aming Komunidad</span>
      </div>

      <div class="anim-3 text-center mb-2">
        <h1 class="font-display text-5xl md:text-6xl font-light text-bark leading-[1.1] tracking-tight">Gumawa ng</h1>
        <h1 class="font-display text-5xl md:text-6xl font-light italic brand-italic leading-[1.05] tracking-tight">Bagong Account</h1>
      </div>
      ...
      <form method="POST" action="{{ route('register.post') }}" class="anim-4 flex flex-col gap-5">
        @csrf
        ...
        <div class="flex flex-col gap-2">
          <label class="text-xs tracking-[0.2em] uppercase text-bark-mid/70 font-sans font-medium">Buong Pangalan</label>
          <input type="text" name="name" placeholder="Juan dela Cruz" class="input-field" value="{{ old('name') }}" required/>
        </div>
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="relative">
            <input type="password" id="reg-password" name="password" placeholder="••••••••" class="input-field" style="padding-right:44px" required/>
            <button type="button" onclick="togglePassword('reg-password', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-bark-mid/50 hover:text-bark-mid transition-colors" tabindex="-1">
        ...
        <div class="flex flex-col gap-2">
          ...
          <div class="relative">
            <input type="password" id="reg-password-confirm" name="password_confirmation" placeholder="••••••••" class="input-field" style="padding-right:44px" required/>
            <button type="button" onclick="togglePassword('reg-password-confirm', this)" class="absolute right-3 top-1/2 -translate-y-1/2 text-bark-mid/50 hover:text-bark-mid transition-colors" tabindex="-1">
              <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/></svg>
            </button>
          </div>
        </div>
      ...
      <div class="anim-5 text-center mt-7">
        <span class="text-sm text-bark-mid/60 tracking-wide">Mayroon nang account? </span>
        <a href="{{ route('login') }}" class="text-sm text-gold hover:text-rust transition-colors tracking-wide font-medium">Mag-sign in</a>
      </div>
    </div>
  </main>
  ...
  <script>
    function togglePassword(id, btn) {
      ...
      btn.querySelector('svg').innerHTML = isHidden
        ? '<path stroke-linecap="round" stroke-linejoin="round" d="M13.875 18.825A10.05 10.05 0 0112 19c-4.477 0-8.268-2.943-9.542-7a9.97 9.97 0 012.163-3.592M6.53 6.533A9.956 9.956 0 0112 5c4.478 0 8.268 2.943 9.542 7a10.025 10.025 0 01-4.132 5.411M3 3l18 18"/><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 01-4.243 2.757M9.88 9.878A3 3 0 0115 12"/>'
